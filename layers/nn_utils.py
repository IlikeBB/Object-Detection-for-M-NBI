...
import layers.OCtaveResnet as octnet
import layers.densenet as densenet
import layers.mobilenetv3 as mobilenetv3
...
class BasicConv(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=0, dilation=1, 
        ...
        self.relu = nn.LeakyReLU(inplace=True) if relu else None#ReLU
...
def vgg(cfg, i, batch_norm=False):
    ...
    for v in cfg:
        if v == 'M':
            ...
            if batch_norm:
                layers += [conv2d, nn.BatchNorm2d(v), nn.LeakyReLU(inplace=True)]#ReLU
            else:
                layers += [conv2d, nn.LeakyReLU(inplace=True)]#ReLU
            in_channels = v
    pool5 = nn.MaxPool2d(kernel_size=3, stride=1, padding=1)
    conv6 = nn.Conv2d(512, 1024, kernel_size=3, padding=6, dilation=6)
    conv7 = nn.Conv2d(1024, 1024, kernel_size=1)
    layers += [pool5, conv6,
               nn.LeakyReLU(inplace=True), conv7, nn.LeakyReLU(inplace=True)]#ReLU
    return layers
...
########################Made by Jess@@@@@@@@@@@@@
    elif backbone_name in octnet.__all__: 
        return getattr(octnet,backbone_name)(pretrained=False)
    elif backbone_name in densenet.__all__: 
        return getattr(densenet,backbone_name)(pretrained=False)
    elif backbone_name in mobilenetv3.__all__: 
        return getattr(mobilenetv3,backbone_name)(pretrained=False)
...
class SFAM(nn.Module):
    def __init__(self, planes, num_levels, num_scales, compress_ratio=16):
        ...
        self.fc1 = nn.ModuleList([nn.Conv2d(self.planes*self.num_levels,
                                                 self.planes*self.num_levels // compress_ratio,
                                                 1, 1, 0)] * self.num_scales)
        self.relu = nn.LeakyReLU(inplace=True)
        self.fc2 = nn.ModuleList([nn.Conv2d(self.planes*self.num_levels // compress_ratio,
...
#     el
    if key == 'base_out':
        assert len(value) <5, 'We have to ensure that the base feature is formed with 2 backbone features'
    elif key == 'planes':
