...
def init_net(net, cfg, resume_net):    
    if cfg.model.init_net and not resume_net:
        ...
        if cfg.test_cfg.cuda:
            ...
            state_dict = torch.load(resume_net, map_location=lambda storage, loc: storage)
...
def adjust_learning_rate(optimizer, gamma, epoch, step_index, iteration, epoch_size, cfg): ##UNSOLVED!!! 
    global lr
    if epoch <= 5:
        lr = cfg.train_cfg.end_lr + (cfg.train_cfg.lr[0]-cfg.train_cfg.end_lr)\
         * iteration / (epoch_size * cfg.train_cfg.warmup)
    else:
        for i in range(len(cfg.train_cfg.step_lr.VOC)):  #ONLY COCO
            if cfg.train_cfg.step_lr.VOC[i]>=epoch:
...
def print_train_log(iteration, print_epochs, info_list):
    if iteration % print_epochs == 0:
        cprint('Time:{}||Epoch:{}||EpochIter:{}/{}||Iter:{}||Loss_L:{:.4f}||Loss_C:{:.4f}||Batch_Time:{:.4f}||LR:{:.7f}'.format(*info_list), 'yellow')
...
def save_checkpoint(net, cfg, final=True, datasetname='COCO',epoch=10, prefix=''):
    if final:
        torch.save(net.state_dict(), cfg.model.weights_save + \
                prefix+'Final_M2Det_{}_size{}_net{}.pth'.format(datasetname, cfg.model.input_size, cfg.model.m2det_config.backbone))
        return cfg.model.weights_save + \
                prefix+'Final_M2Det_{}_size{}_net{}.pth'.format(datasetname, cfg.model.input_size, cfg.model.m2det_config.backbone)
    else:
        torch.save(net.state_dict(), cfg.model.weights_save + \
                prefix+'M2Det_{}_size{}_net{}_epoch{}.pth'.format(datasetname, cfg.model.input_size, cfg.model.m2det_config.backbone,epoch))
        return cfg.model.weights_save + \
                prefix+'M2Det_{}_size{}_net{}_epoch{}.pth'.format(datasetname, cfg.model.input_size, cfg.model.m2det_config.backbone,epoch)
