...
args = []
config = 'configs/m2det704_densenet201.py.py'
dataset_prefix = 'VOC'
ngpu = 1
resume_epoch = 0
if resume_epoch >0:
    resume_net = 'weights/M2Det_VOC_size704_densenet201_epoch%d.pth' % (resume_epoch)
else:
    resume_net=None
tensorboard = False
...
for iteration in range(start_iter, max_iter):
    if iteration % epoch_size == 0:
        ...
        if epoch>0 and (epoch % cfg.model.save_eposhs == 0):
            save_checkpoint(net, cfg, final=False, datasetname = dataset_prefix, epoch=epoch, prefix='')
