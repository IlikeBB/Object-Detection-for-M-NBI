...
class M2Det(nn.Module):
    ...
    def construct_modules(self,):
        ...
        elif 'dense' in self.net_family:
            self.base = get_backbone(self.backbone)
            shallow_in, shallow_out = 512,256
            deep_in, deep_out = 1024,512
        elif 'mobile' in self.net_family:
            self.base = get_backbone(self.backbone)
            shallow_in, shallow_out = 80,128
            deep_in, deep_out = 160,512
            mid_in, mid_out =112,128
            self.mid_reduce= BasicConv(mid_in, mid_out, kernel_size=1, stride=1)
...
#         self.init_reduce= BasicConv(256, 128, kernel_size=1, stride=1)
        ...
        if self.sfam:
            self.sfam_module = SFAM(self.planes, self.num_levels, self.num_scales, compress_ratio=self.cr)
    ...
    def forward(self,x):
        ...
        if 'vgg' in self.net_family:
            for k in range(len(self.base)):
                ...
                if k in self.base_out:
                    base_feats.append(x)
        elif 'res' in self.net_family and 'oct' not in self.net_family:
            import pdb
            
            base_feats = self.base(x, self.base_out)
            base_feature = torch.cat(
                (#F.interpolate(self.init_reduce(base_feats[0]),scale_factor=0.5),
                 self.reduce(base_feats[0]), 
                 F.interpolate(self.up_reduce(base_feats[1]),scale_factor=2)
                 ),1)
        elif 'oct' in self.net_family:
            ################USE OCTCONV ####################
            base_feats = self.base(x, self.base_out)
            base_feature = torch.cat(
                 (self.reduce(torch.cat(((base_feats[0][0]), (F.interpolate(base_feats[0][1],scale_factor=2))),1)), 
                 self.mid_reduce(torch.cat(((base_feats[1][0]), (F.interpolate(base_feats[1][1],scale_factor=2))),1)),
                 F.interpolate(self.up_reduce(base_feats[2]),scale_factor=2)
                 ),1)
        elif 'dense' in self.net_family: # Including ResNet series and ResNeXt series
            base_feats = self.base(x, self.base_out)
            import pdb
            pdb.set_trace()
            base_feature = torch.cat(
        ...
        elif 'mobile' in self.net_family: # Including ResNet series and ResNeXt series
            base_feats = self.base(x, self.base_out)
            
            base_feature = torch.cat(
                (self.reduce(base_feats[0]), 
                 self.mid_reduce(base_feats[1]), 
                 F.interpolate(self.up_reduce(base_feats[2]),scale_factor=2)
                 ),1)
    ...
    def init_model(self, base_model_path):
        ...
        self.mid_reduce.apply(weights_init)
#         self.init_reduce.apply(weights_init)
